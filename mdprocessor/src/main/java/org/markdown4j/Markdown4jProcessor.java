...
public class Markdown4jProcessor {
	...
	private Builder builder() {
		...
		return Configuration.builder().forceExtentedProfile().registerPlugins(new TodoListPlugin(), new YumlPlugin(), new WebSequencePlugin(), new IncludePlugin()).convertNewline2Br().setDecorator(decorator).setCodeBlockEmitter(new CodeBlockEmitter());
	}
